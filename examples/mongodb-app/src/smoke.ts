...
import { seedIds } from './schema.service';
...
async function main(): Promise<void> {
  ...
  const relationResponse = await server.inject({
    method: 'GET',
    url: `/meta/users/${seedIds.ada.toHexString()}?with=profile,posts,roles&select%5Bprofile%5D=bio&select%5Bposts%5D=title&select%5Broles%5D=name`,
  });
  ...
  console.log('MongoDB example smoke test passed');
...
function assertRelations(user: any): void {
  ...
  if (user.profile._id !== undefined) {
    throw new Error(
      `Profile _id should not be selected: ${JSON.stringify(user)}`,
    );
  }
  ...
  if (user.posts[0]._id !== undefined) {
    throw new Error(`Post _id should not be selected: ${JSON.stringify(user)}`);
  }
  ...
  if (user.roles[0]._id !== undefined) {
    throw new Error(`Role _id should not be selected: ${JSON.stringify(user)}`);
  }
}
